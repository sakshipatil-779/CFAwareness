...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    ...
    const userEntries = stored.map((s) => ({
      ...
      totalEcoPoints: s.totalEcoPoints,
      totalCarbonSaved: s.totalCarbonSaved,
      country: '🌍',
      isCurrentUser: true,
    }));
    ...
    <div className="page-container py-10">
      <div className="mx-auto max-w-3xl">
        ...
        <div className="mb-8 text-center">
          <div className="eco-badge animation-fill-both mb-4 animate-fade-in justify-center">
            <span aria-hidden="true">🌍</span>
            Live Rankings — EcoQuest
          </div>
          <h1 className="gradient-text animation-fill-both animate-delay-100 mb-3 animate-slide-up font-display text-4xl font-extrabold">
            {t('lb.title')}
          </h1>
          <p className="animation-fill-both animate-delay-200 animate-slide-up text-slate-500">
        ...
        {!isLoading && entries.length >= 3 && (
          <div
            className="animation-fill-both animate-delay-200 mb-8 grid animate-slide-up grid-cols-3 gap-3"
          ...
          >
            ...
            <div className="flex flex-col items-center gap-2 pt-6">
              <div
                ...
                style={{ border: '1.5px solid rgba(148,163,184,0.30)' }}
              >
            ...
            <div className="flex flex-col items-center gap-2">
              <div
                className="glass-card w-full animate-glow-pulse p-5 text-center"
                style={{
              ...
              >
                ...
                <p className="truncate font-display font-bold text-slate-800">
                  {entries[0]?.displayName}
                </p>
                <p className="font-display text-xl font-extrabold" style={{ color: '#15803d' }}>
                  {entries[0]?.totalEcoPoints} pts
                </p>
            ...
            <div className="flex flex-col items-center gap-2 pt-6">
              <div
                className="glass-card w-full p-4 text-center"
                style={{ border: '1.5px solid rgba(194,65,12,0.20)' }}
              >
                <p className="mb-1 text-3xl" aria-hidden="true">
                  🥉
                </p>
            ...
            <button
              ...
              className={`rounded-xl px-4 py-2 text-sm font-semibold transition-all duration-200 ${
                filter === f
                  ...
                  : 'text-slate-400 hover:bg-eco-50/50 hover:text-eco-600'
              }`}
            >
        ...
        <div className="glass-card overflow-hidden">
          {/* Header row */}
          <div
            className="grid grid-cols-[3rem_1fr_6rem_7rem] gap-3 px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-400"
            style={{ borderBottom: '1px solid rgba(34,197,94,0.08)' }}
          >
            <span>{t('lb.rank')}</span>
            <span>{t('lb.player')}</span>
            <span className="text-right">{t('lb.points')}</span>
            <span className="hidden text-right sm:block">{t('lb.carbon')}</span>
          </div>
          ...
          {isLoading ? (
            <div aria-busy="true" aria-label="Loading leaderboard">
              {Array.from({ length: 8 }).map((_, i) => (
                <div
                  key={i}
                  className="grid grid-cols-[3rem_1fr_6rem_7rem] items-center gap-3 px-5 py-4"
                ...
                >
                  ...
                  />
                  <div className="ml-auto h-4 w-14 animate-pulse rounded-lg bg-slate-100" />
                  <div className="ml-auto hidden h-4 w-16 animate-pulse rounded-lg bg-slate-100 sm:block" />
                </div>
              ))}
            </div>
          ) : displayed.length === 0 ? (
            <div className="py-14 text-center">
              ...
              <p className="mb-4 text-slate-500">No scores yet! Play EcoQuest to appear here.</p>
            ...
            <div>
              {displayed.map((entry, i) => {
                ...
                return (
                  <div
                    key={entry.uid}
                    className="animation-fill-both grid animate-slide-up grid-cols-[3rem_1fr_6rem_7rem] items-center gap-3 px-5 py-4 transition-colors duration-150 hover:bg-eco-50/50"
                    style={{
                      borderBottom:
                        i < displayed.length - 1 ? '1px solid rgba(34,197,94,0.05)' : 'none',
                  ...
                  >
                    <span
                      ...
                      style={{ color: rankStyle?.color ?? '#94a3b8' }}
                    >
                      {rankStyle ? rankStyle.icon : `#${entry.rank}`}
                    </span>
                    <div>
                      <p className="flex flex-wrap items-center gap-1.5 text-sm font-semibold text-slate-700">
                        {entry.country && <span aria-hidden="true">{entry.country}</span>}
                        {entry.displayName}
                        {entry.isCurrentUser && (
                          <span className="eco-badge px-1.5 py-0.5 text-[10px]">You</span>
                        )}
                      </p>
                    </div>
                    <div className="text-right">
                      <span className="font-display font-bold text-eco-600">
                    ...
                    <div className="hidden text-right sm:block">
                      <span className="text-sm font-medium text-teal-600">
                        {formatCarbon(entry.totalCarbonSaved)}
                      </span>
                    </div>
                  </div>
                );
        ...
        {/* Play CTA */}
        <div className="animation-fill-both animate-delay-500 mt-8 animate-fade-in text-center">
          <Link to="/game" id="lb-play-btn" className="btn-primary px-10 py-4 text-lg">
            <span aria-hidden="true">🚀</span>
            {t('home.cta.play')}
          </Link>
          <p className="mt-3 text-xs text-slate-400">
            Complete all scenarios to appear on the leaderboard
          </p>
        </div>
      </div>
    </div>
